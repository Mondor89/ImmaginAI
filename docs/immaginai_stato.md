...
| Campo | Valore |
|-------|--------|
| Ultimo aggiornamento | 2 Settembre 2026 — S24 |
| Versione app | v4.4 |
| Stato | ✅ Pollinations primario (gratis, illimitato) — CF come backup qualità (steps:4), Together.ai disattivato via gate esplicito. Sessione D del backlog S21 (Pulizia tecnica) in corso: dead code JS+CSS rimosso, listener leak fixato, collisione colori DV(cyan)/Stile(viola) risolta, CSS morto del vecchio admin pre-S11 rimosso da `immaginai_light.css` |
| Prossima task | Sessione D — resta da fare: unificare i 3 blocchi generate*/regen* quasi identici, audit manuale selettore-per-selettore del resto del CSS morto, unificare i 5 colori del pannello admin — vedi "Backlog per sessioni future" sotto |
...
### Sessione D — Pulizia tecnica: dead code + CSS [bassa, zero rischio, sessione dedicata a parte] — ⏳ in corso, iniziata S24
...
- [ ] **Resta da fare** (rimandato — non zero-rischio come il resto della sessione, richiede QA visiva più estesa): unificare i 3 blocchi quasi identici `generateImage`/`generateModifica`/`regenImage` (~90 righe duplicate), audit CSS morto in `immaginai_light.css` (~46 regole, 99 `!important`) e selettori duplicati non conflittuali, 5 colori diversi nel pannello admin da uniformare, collisione di significato sui colori attivi (DV cyan vs Stile/Formato viola appiattiti dal tema light)
