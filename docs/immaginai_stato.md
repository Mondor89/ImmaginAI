...
| Versione app | v4.4 |
| Stato | ✅ Pollinations primario (gratis, illimitato) — CF come backup qualità (steps:4), Together.ai disattivato via gate esplicito. CLAUDE.md allineato al template APP (U-039→U-052 recepiti S21). Sessioni A e B del backlog S21 chiuse in S22: `generate.js` validato/con timeout/HF rimosso, download/galleria/ciclo Modifica/resize mobile sistemati |
...
| Sessione | Attività |
|----------|----------|
| S22 | Sonnet 5 impegno medio, Sessioni A+B del backlog S21 nella stessa sessione di lavoro (REGISTRA a metà per chiuderle separatamente). **A** — Backend/sicurezza `generate.js`: validazione input, timeout 8s per provider, rimosso HuggingFace (dominio morto, confermato via `curl`), gate `TOGETHER_ENABLED`, pin `NODE_VERSION=20`. Non verificabile in Browser pane (secret server-side) — `node --check` + test manuale di `clampDim()`. **B** — Flusso UX: trovato e fixato un bug reale di hang infinito in `upscaleCanvas()` (tainted canvas senza try/catch), download più affidabile su Pollinations, `saveGallery()` con gestione `QuotaExceededError`, limite 12 immagini esplicito, `#statusBar` riattivato (scelta di Fabio), ciclo Modifica chiuso, resize mobile con gating anti-tastiera-virtuale. Tutto verificato dal vivo in Browser pane (immagine reale generata, download testato, login admin, screenshot, simulazione resize) |
