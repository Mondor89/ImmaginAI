...
- [x] Patch scritta in `CLAUDE.md` → "Regole JavaScript / Web — pattern-trappola": nuova voce "Promise + callback (DOM, timer, reader)", con le 3 correzioni dell'audit (attribuzione accurata, chiarito che `crossOrigin` da solo non basta e forza una richiesta separata, generalizzata oltre i callback DOM e collegata esplicitamente al principio già esistente "un'attesa senza limite non fallisce, si pianta"). **AMBITO: da portare nel template** (pattern JS trasferibile) — depositare in `patch/_inbox` di Template Claude in una prossima sessione dedicata, non fatto qui
