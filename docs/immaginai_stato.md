...
| Versione app | v4.4 |
| Stato | ✅ Pollinations primario (gratis, illimitato) — CF come backup qualità (steps:4), Together.ai disattivato via gate esplicito. Sessione D del backlog S21 (Pulizia tecnica) in corso: dead code JS rimosso in app+admin, CSS morto rimosso, listener leak fixato, verificato dal vivo desktop+mobile+admin |
| Prossima task | Sessione D — resta da fare: unificare i 3 blocchi generate*/regen* quasi identici, audit CSS admin (5 colori diversi), collisione colore attivi DV/Stile — vedi "Backlog per sessioni future" sotto |
...
- [ ] **Resta aperto** (rimandato, non zero-rischio): unificare i 3 blocchi `generateImage`/`generateModifica`/`regenImage`, audit CSS morto in `immaginai_light.css`, 5 colori diversi nel pannello admin, collisione colori attivi DV/Stile — richiedono QA visiva più estesa, sessione a parte
...
| Sessione | Attività |
|----------|----------|
| S24 | Sonnet 5 impegno base, Sessione D del backlog S21 (Pulizia tecnica: dead code + CSS) — iniziata, non chiusa. Rimossi `index.html`+loghi duplicati in root, dead code JS verificato con grep in app+admin (funzioni no-op, `BRAND_PALETTES`, `CAT_ID_ATTIVI`, `ST.enhance`/`safe`/`activeApi`), CSS morto (accordion "Opzioni avanzate", variabili mai lette con `var()`). Trovati durante la verifica (non nel backlog originale): `liveBtn`/`loadBtnColors` dead code, leak reale di listener scroll in `buildDvTabs()`. Fix minori: `generateModifica` disabilita i bottoni Genera, guardia NaN estesa al prefill admin, `HF_TOKEN`→`HORDE_API_KEY`. Verificato dal vivo (desktop+mobile+admin), nessuna regressione. Resta aperto: unificare i 3 blocchi generate*/regen*, audit CSS `immaginai_light.css`, colori admin — rimandati a sessione dedicata (non zero-rischio) |
