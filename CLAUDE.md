...
- **Verificare l'API di una dipendenza di terze parti non basta: controllare anche se esegue attese o I/O bloccante al momento del caricamento del modulo, non solo dentro le sue funzioni pubbliche.** Adattato al contesto browser di ImmaginAI: un `import()` dinamico (es. `loadBgRemoval()`/`loadUpscaler()`) può nascondere un'attesa bloccante dentro l'inizializzazione del modulo stesso, non solo nella funzione che poi si chiama — verificarlo prima di considerare il caricamento "veloce perché lazy". La mitigazione è diversa da quella di un'attesa senza limite nel proprio codice, perché qui il codice non è tuo: va creata la precondizione **prima** del caricamento, oppure il caricamento va isolato in un contesto che può fallire rumorosamente (timeout/`catch` sull'`import()`, come già fanno `loadBgRemoval()`/`loadUpscaler()` con i loro codici d'errore dedicati). *Distinto dal bullet sul calcolo pesante che blocca il thread: lì blocca la funzione che chiami, qui il modulo mentre si carica — e nessuno dei due lo rivela il solo controllo dell'API pubblica.*
- **Un controllo automatico (hook, script, guardia) che decide se scattare confrontando un file di progetto con una frase esatta è fragile in modo silenzioso** — un fraseggio diverso nel progetto (o una riformulazione futura) lo disattiva senza errore visibile. Preferire l'ancora più stabile disponibile (un titolo di sezione, non una frase intera) e, quando serve confermare che il controllo funziona davvero, verificarlo sul progetto reale — non assumerlo dalla sintassi corretta. *Caso reale: l'hook globale Claude Code che ricorda di dichiarare modello/istanze dei sotto-agenti cercava `grep -q "sotto-agenti sono la terza leva"` — non è mai scattato su questo progetto, che scrive "sono **una** terza leva".*
...
- **Quando la dimensione conta, preferire `blob:`/`createObjectURL` a `data:`/base64** — evita il +33% e non serve quando il consumo resta lato client (canvas, upscale, download): `data:` serve solo quando i byte devono viaggiare come stringa JSON verso un endpoint server (Kontext, background-removal). `downloadFrom()` in `Immaginai.html` usa già `createObjectURL` per l'upscale locale — stesso principio, già applicato lì prima di essere scritto qui come regola.
